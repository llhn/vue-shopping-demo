<template>
  <div class="prodetail" v-if="detailList">
    ...
    <div class="info">
      <div class="title">
        <div class="price">
          <span class="now">￥{{ detailList.goods_price_min
          }}</span>
    ...
    <div class="comment">
      <div class="comment-title">
        <div class="left">商品评价 ({{ commitList.length }}条)</div>
        <div class="right">查看更多 <van-icon name="arrow" /> </div>
      </div>
      <div class="comment-list">
        <div class="comment-item" v-for="item in commitList" :key="item.commit_id">
          <div class="top">
    ...
    <!-- 点击加入购物车或立即购买 唤起弹窗 -->
    <van-action-sheet v-model="isShow" :title="mode === 'cart' ? '加入购物车' : '立刻购买'">
      <div class="product">
...
import { getDetailData, getProductData } from '@/api/detail'
...
export default {
  ...
  methods: {
    ...
    //将请求数据封装
    async getDetailList() {
      const res = await getDetailData(this.goodsId)
      console.log(res);
    ...
    //获取评价
    async getCommit() {
      const res = await getProductData(this.goodsId, 5)
      console.log(res);
    ...
    async addCart() {
      ...
      //添加到购物车
      const { data } = await addCart(this.goodsId, this.count, this.detailList.skuList[0].goods_sku_id)
